...
**いま `rules/rules.json` は「実測勝率なし」で生成されている。**
`api.brawlify.com` がこのネットワークから Cloudflare に弾かれ（403）、
フォールバック先のミラー `api.brawlapi.com` はローテーション（`/events`）と
マップ別勝率（`/maps/<id>` の `stats`）を空で返すため。

- キャラ・マップ・モードのメタデータは問題なく取れている（マップ 159 / キャラ 109）
- 勝率が無い間は**役割適性だけ**でスコアリングされ、同じ役割のキャラが同点になる。
  並び順は名前順なので**順位に意味は無い**（rules.json の `dataQuality.tiebreak` が
  `"alphabetical"` になり、アプリにもオレンジ色で警告が出る）
- **GitHub Actions のランナーからは `api.brawlify.com` へ直接届く可能性が高い。**
  ワークフローを 1 回手動実行して、サマリーの「実測勝率: あり」を確認するのが最初の一歩。
  `--require-stats` を付けると、取れなかったときに失敗させられる
- 勝率が取れるまでの間は `data/tier_overrides.json` に自分の感覚を入れると実用的になる

その他:

- **枠位置の既定値は推測値**。実機のスクショに合わせた枠合わせは必須（§6）
